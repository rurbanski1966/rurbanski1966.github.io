-- ============================================================================
-- Lana — migration 025: SMC Weekly spend on the Dashboard
--
-- Additive. Run after 024. Safe to re-run.
--
-- Per Ryan 2026-09-20: same idea as 024's smc_daily_spend, but for the
-- Monday-Sunday week instead of today — same two cost sources (AI grading,
-- AI review/manual-summary), same SMC team resolution (call's own team_id
...
-- migrations 015, 018, 022, 023 and 024 needed it).
-- ============================================================================
...
as $$
  ...
  -- Every call_scores row with its resolved team name, so the SMC tiles below
  -- are plain filters on this instead of repeating the two joins per query.
  scored_teamed as (
    select
      cs.cost_usd,
